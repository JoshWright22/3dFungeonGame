...
public class GameNetworkManager : MonoBehaviour
{
	...
	public void StartClient(SteamId id)
	{
		...
		{
			Debug.LogWarning("StartClient called while already connected - ignoring.", this);
			return;
		}
	...
	{
		// Only join the Steam lobby here. Actually starting the Netcode client is OnLobbyEntered's
		// job - calling StartClient from both paths connected twice on every invite-accept.
		RoomEnter joinResult = await lobby.Join();

		if (joinResult != RoomEnter.Success)
			Debug.LogError($"Failed to join lobby {lobby.Id}: {joinResult}", this);
	}
